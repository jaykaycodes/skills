...
3. **Run impeccable teach (REQUIRED)**: If neither source has context, you MUST run /ui teach NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
If this skill is invoked with the argument "craft" (e.g., `/ui build [feature description]`), follow the [craft flow](reference/craft.md). Pass any additional arguments as the feature description.

---

## Teach Mode

If this skill is invoked with the argument "teach" (e.g., `/ui teach`), skip all design work above and instead run the teach flow below. This is a one-time setup that gathers design context for the project.
...
If this skill is invoked with the argument "extract" (e.g., `/ui tune layout [target]`), follow the [extract flow](reference/extract.md). Pass any additional arguments as the extraction target.